...
$con = connect();
    ...
    if (isset($_POST['book'])) {
       ...
       if ($bdinsert == true && $cinsert == true && $iinsert == true) {
    		...
    		echo '<script>window.location="pay.php?itmname='.$item_name.'&phone='.$reservation_phone.'&date='.$reservation_date.'&time='.$reservation_time.'&price='.$total_price.'&email='.$reservation_email.'"</script>';
    	}else {
    		echo "Error: " . $iquery . "<br>" . $con->error;
    		echo "Error: " . $ciQuery . "<br>" . $con->error;
            echo "Error: " . $iquery . "<br>" . $con->error;
